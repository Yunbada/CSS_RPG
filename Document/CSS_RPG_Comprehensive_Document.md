...
*   **[Phase 6: 게임 모드화 (Round Manager 고도화)]**: 인간 vs 좀비 진형을 나누어 숙주 좀비 감염 등의 라운드 기반 규칙 정립.
*   **[Phase 7: 전용 서버 기반 데이터 중앙화 및 로그인 시스템 고도화 (Dedicated Server Architecture)]**:
    1.  **로그인 UI 개편 및 서버 상태 확인**: 로그인 화면 우측 상단 IP 입력 칸 하단에 **[서버 접속]** 버튼 추가. 이 버튼을 누르기 전까지는 로그인/회원가입 불가. 
    2.  **서버 상태 인디케이터**: [서버 접속]을 눌러 서버의 생존 여부(ping/연결 상태)를 확인. 가동 중이면 IP 칸 왼쪽에 **초록색 동그라미(🟢)**, 비가동 중이면 **빨간색 동그라미(🔴)** UI 표시.
    3.  **데이터베이스 중앙화 (C/S 구조)**: 로그인/회원가입 로직을 로컬 검증이 아닌 켜져 있는 서버 쪽에 인증 및 생성 요청을 보내도록 변경. (가동된 서버만이 DB 검증 및 쓰기를 수행)
    4.  **서버 권한 데이터 동기화**: 접속 이후의 모든 플레이어 정보상태, 인벤토리 아이템 등의 핵심 데이터는 조작 방지를 위해 **무조건 가동된 서버가 가진 DB 데이터**만을 로드하고 신뢰하도록 아키텍처 전면 수정.
